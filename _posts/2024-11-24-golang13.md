...
단순 echo에 대한 tcp 서버 전체 구현은 다음과 같습니다. 여기서 각각 코드가 무슨 역할을 하는지 살펴보겠습니다.
...
![image.png](https://prod-files-secure.s3.us-west-2.amazonaws.com/641cb245-6828-454d-a10e-221004d3c4c4/b6341553-1378-4564-8496-0cbf5bc65966/image.png)

해당 api 호출 파이프라인을 보면 다음의 단계를 따릅니다. 위의 일부 syscall은 인자로 fd라는 값을 활용하는데 여기서는 이런 syscall이 있다는 정도로 보겠습니다. 이후의 아티클에서 해당 syscall을 golang에서 활용하여 서버를 구성하는 방법도 소개해보겠습니다.
...
해당 코드는 net.Listen이라는 net 패키지의 Listen 함수를 호출하여 4계층 프로토콜과 주소를 선언하면 해당 인자에 맞는 net.Listener인터페이스를 리턴합니다. Listen 메서드는 위에서 설명한 POSIX syscall의 socket(), bind(), listen()을 추상화합니다. 사용자는 net.Listen을 호출하여 서버측 소켓을 생성하고 사용자가 입력한 주소에 소켓을 바인딩하고 소켓 상태를 네트워크 연결할 수 있는 상태로 변경합니다.
...
뭔가 고루틴의 개수를 증가시켜 구현한다는 점이 찝찝해보입니다. 일단 golang이 경량 쓰레드로 불린다지만 연결마다 고루틴이 생성됩니다. 일단 찝찝함을 뒤로 하며 이 정도 특징만 정리하고 다음에는 직접 POSIX syscall을 호출하여 서버를 구현해봅시다. POSIX syscall을 이용한 직접 구현에는 소켓을 애플리케이션 계층에서 어떻게 다루게 되는지 살펴보겠습니다.
