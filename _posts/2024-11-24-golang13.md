...
해당 api 호출 파이프라인을 보면 다음 단계를 따릅니다. 일부 syscall은 fd를 인자로 활용하는데, 이번에는 이런 syscall이 있다는 정도만 살펴보겠습니다. 이후 아티클에서 이 syscall들을 golang에서 직접 활용하는 방법도 소개하겠습니다.
...
listener.Accept을 호출하면 클라이언트의 연결 요청을 수락하고 net.Conn 인터페이스로 리턴합니다. 개발자는 이 net.Conn 인터페이스의 메서드를 호출하여 클라이언트와 통신할 수 있습니다.

다음은 accept 내부의 고루틴을 선언하여 비동기로 R/W하는 부분입니다.

```go
// https://github.com/golang/go/blob/a10e42f219abb9c5b/src/net/net.go#L175
		// ...
		go func(conn net.Conn) {
			b := make([]byte, 1024)
			for {
				r, err := conn.Read(b) // ✅ 데이터 수신까지 블로킹
				if err != nil {
					fmt.Println("read error", err)
					break
				}

				_, err = conn.Write(b[:r]) // ✅ 수신 데이터 에코
				if err != nil {
					fmt.Println("write error", err)
					break
				}
...
여기서 net.Conn 인터페이스를 인자로 받아 `conn.Read(b)`에서 클라이언트의 데이터가 수신될 때까지 고루틴을 블로킹합니다. Read에서 블로킹이 발생하므로 각 conn 처리는 고루틴을 생성하여 비동기로 처리합니다. 따라서 부모 컨텍스트의 루프는 바로 다시 Accept로 돌아가서 다른 클라이언트 연결을 대기할 수 있습니다. 만약 클라이언트가 소켓에 데이터를 썼다면 `conn.Read(b)`의 블로킹이 해제되고 r에는 b에 저장된 데이터의 크기가 리턴됩니다.
