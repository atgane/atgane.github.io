---
title: "kubernetes data plane: containerd sandbox 생성 과정 추적"
...
tags:
  ...
  - CNI
  - shim
excerpt: "kubelet의 RunPodSandbox 요청에 대한 containerd 내부 처리 과정을 netns 생성부터 shim 기동까지 코드 수준으로 분석합니다."
toc: true
toc_sticky: true
...
이전 포스트에서는 blank import를 통한 플러그인 사전 등록과 `server.New()`에서의 gRPC 서버 구성까지 살펴봤습니다. 이번 편에서는 kubelet이 이 gRPC 서버에 CRI 요청을 보냈을 때 어떤 일이 일어나는지 추적합니다. `GRPCPlugin "cri"` 플러그인의 등록 구조부터 `RunPodSandbox` 처리 과정까지 단계별로 살펴보겠습니다.
...
샌드박스가 Ready 상태로 전환되면 kubelet은 곧이어 `CreateContainer`를 호출하여 실제 워크로드 컨테이너를 추가합니다. 다음 편에서는 `CreateContainer`와 `StartContainer`가 이 샌드박스 위에서 컨테이너 프로세스를 어떻게 준비하고 실행하는지 살펴봅니다.
