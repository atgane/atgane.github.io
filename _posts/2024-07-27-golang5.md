...
이번에는 golang에서 channel만큼 중요한 select문의 동작 원리에 대해 탐구해보겠습니다. 점점 뭔가 동시성을 활용한 구현을 못하고 있는 느낌인데… select문 내부 동작에서도 재미있는 부분이 많기 때문에 이 부분도 학습하면 동시성을 이해하는 과정에 도움이 될 것이라 생각합니다.
...
어떤 작업을 하는 `doSomething` 이라는 함수가 있고 해당 함수는 외부 훅을 호출합니다. 이때 훅에 대해서 `doSomething` 함수 외부에서 전달되는 취소를 위의 예시처럼 `context` 패키지를 이용하여 감지할 수 있습니다. 여기는 context에 대해서 자세히 다루지 않고 이런 예시로 사용할 수 있다는 정도로 알아둡시다. 해당 패키지도 동시성 처리에 굉장히 유용하기에 나중에 다루도록 하겠습니다. 여기서 중요한 점은 select문도 채널만큼 golang 내부에서 활용하기가 좋다는 것입니다. 그러나 select문으로 채널을 이용하다 보면 아래와 같은 궁금점이 생깁니다. 
...
https://github.com/golang/go/blob/3959d54c0bd5c92fe0a5e33fedb0595723efc23b/src/runtime/select.go#L121
...
```go
	...
	// select 문 최적화: 컴파일러는 0 또는 1개의 case & default만 있는 상황을 단순하게 변경
```

컴파일러는 그럼 단순한 경우를 어떻게 최적화 할까요? 이 경우에 대한 정답은 다시 채널 내부 코드에서 확인할 수 있습니다. 

https://github.com/golang/go/blob/3959d54c0bd5c92fe0a5e33fedb0595723efc23b/src/runtime/chan.go#L736
...
```go
	...
	norder := 0
	for i := range scases { // 각 case 문 검사
		cas := &scases[i]

		// Omit cases without channels from the poll and lock orders.
		if cas.c == nil { // 채널이 없으면 생략
			cas.elem = nil // allow GC
			continue
		}

		if cas.c.timer != nil { // 타이머의 경우 타이머를 실행할 수 있게 maybeRunChan 호출
			cas.c.timer.maybeRunChan()
		}

		// Fisher-Yates 셔플
		// pollorder 배열에 무작위로 추가
		j := cheaprandn(uint32(norder + 1)) // 무작위 숫자를 생성하고 case문 순서 셔플
		pollorder[norder] = pollorder[j]    // pollorder의 norder 자리에 j 자리의 값을 넣고
		pollorder[j] = uint16(i)            // j 자리에는 i를 삽입
		norder++
	}
...
여기서 왜 순서를 랜덤으로 섞는 걸까요? 만약 select문이 대기하는 케이스가 여러 채널에 대해서 동작하고 있고 채널이 모두 당장 실행될 수 있는 경우, 먼저 선언된 케이스만 동작하게 됩니다. 따라서 이 경우 **뒷 케이스의 채널은 스케줄러를 할당받지 못하는 기아 상태가 발생**하게 됩니다. 이런 경우를 방지하기 위해 랜덤으로 케이스 탐색 순서를 섞어 진행합니다.
...
```go
	...
	// lockorder로 lock 순서를 최대힙으로 정렬
	for i := range lockorder {
		j := i
		// Start with the pollorder to permute cases on the same channel.
		c := scases[pollorder[i]].c
		for j > 0 && scases[lockorder[(j-1)/2]].c.sortkey() < c.sortkey() { // 채널을 가르키는 포인터를 기준으로 정렬
			k := (j - 1) / 2
	...
	// lockorder를 힙에서 하나씩 꺼내고 lock 배열 정렬
	for i := len(lockorder) - 1; i >= 0; i-- {
		o := lockorder[i]
		c := scases[o].c
		lockorder[i] = lockorder[0] // 맨 뒤의 원소랑 힙의 맨 처음 요소랑 비교하여 힙 정렬
		j := 0
		for {
			k := j*2 + 1
			if k >= i {
...
정렬된 lockorder를 이용하므로 채널의 주소에 따라 select문의 case를 정렬합니다. 따라서 해당 순서대로 채널 대기로직을 실행하게 되고 모든 고루틴의 select문의 케이스에서 c1 채널을 먼저 확인하게 됩니다. 이렇듯 **select문은 힙 정렬을 활용하여 채널 대기 순서를 채널 포인터 크기로 정렬한 다음 락을 획득하는 방식으로 데드락 문제를 회피**했습니다.
...
```go
	...
	for _, casei := range lockorder {
		...
		cas = &scases[casi]
		c = cas.c
		sg := acquireSudog() // case 개수 만큼 sudog 생성 & 필드 세팅 & sendq, recvq에 등록
		sg.g = gp
		sg.isSelect = true
...
2. select문에 대하여 순서를 셔플합니다: 이 이유는 **성공한 케이스가 여럿인 경우 뒤에 선언된 케이스 문의 기아 상태를 방지**하기 위해서 입니다.
3. lockorder를 정렬하여 락 순서를 정렬합니다: 이 이유는 **여러 채널이 여러 select문에 걸쳐 동작하는 경우 데드락이 방지하는 경우를 방어**하기 위해서 입니다.