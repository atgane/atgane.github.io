...
![golang  비동기 시리즈 비공개 (1).png](https://prod-files-secure.s3.us-west-2.amazonaws.com/641cb245-6828-454d-a10e-221004d3c4c4/6d9e52a3-df7d-45b5-a71c-516cf8da2938/golang__%EB%B9%84%EB%8F%99%EA%B8%B0_%EC%8B%9C%EB%A6%AC%EC%A6%88_%EB%B9%84%EA%B3%B5%EA%B0%9C_(1).png)

epoll을 사용하기 위해 epoll을 생성합니다. 애플리케이션에서 epoll_create, epoll_create1함수를 이용하여 epoll 자료구조를 커널에 생성할 수 있습니다.

![golang  비동기 시리즈 비공개 (2).png](https://prod-files-secure.s3.us-west-2.amazonaws.com/641cb245-6828-454d-a10e-221004d3c4c4/59a401f7-6ca5-4473-82be-65e83599811e/golang__%EB%B9%84%EB%8F%99%EA%B8%B0_%EC%8B%9C%EB%A6%AC%EC%A6%88_%EB%B9%84%EA%B3%B5%EA%B0%9C_(2).png)

epoll을 생성했다면, 생성한 epoll에 소켓 fd를 등록합니다. 이때 epoll_ctl을 이용하여 epoll에 소켓 fd를 등록합니다. 이때 소켓에서 어떤 이벤트를 어떤 방식으로 받을 것인지 선택할 수 있습니다. 자세한 사항은 코드를 보면서 진행하도록 하겠습니다.

![golang  비동기 시리즈 비공개.png](https://prod-files-secure.s3.us-west-2.amazonaws.com/641cb245-6828-454d-a10e-221004d3c4c4/27a76631-012f-4bb7-8753-61c5f26ee701/golang__%EB%B9%84%EB%8F%99%EA%B8%B0_%EC%8B%9C%EB%A6%AC%EC%A6%88_%EB%B9%84%EA%B3%B5%EA%B0%9C.png)
...
epoll_create는 size라는 인자를 전달했으나 리눅스 2.6.8버전부터 무시되기 때문에 사용되지 않습니다. epoll_create1에 인자로 을 호출하면 size가 무시되는 epoll_create와 동일하게 epoll인자를 생성합니다. 단순히 epoll_create1(0)를 이용하여 epoll을 만든다고 보셔도 됩니다.
...
2번 부분 동작을 요약하면 **epoll에 이벤트를 감지할 fd를 등록**하는 것입니다. epoll_ctl의 인자를 확인해보면 epoll fd에 fd에 이벤트 epoll_event를 추가/수정/삭제합니다. 추가/수정/삭제 동작은 op으로 전달하는 인자로 결정됩니다.
...
이 부분에서 발생한 이벤트를 반복문을 통해 처리하게 됩니다. 반복문 내부에서 이벤트의 종류에 따라 소켓 입력인지, 에러 or 종료인지 탐색합니다. 일반 소켓 입력이라면 입력 로직을 처리하는 11번 분기로 진입합니다. 이때 에러가 발생하거나 12번 이벤트가 발생한 경우, 마지막 13번에서 EpollCtl로 소켓 fd에 대한 이벤트 등록을 제거하고 종료합니다.
