...
https://github.com/golang/go/blob/a10e42f219abb9c5b/src/runtime/chan.go#L34
...
type hchan struct {
	...
	recvq    waitq // waiting queue: 수신 대기 큐
	sendq    waitq // waiting queue: 송신 대기 큐
...
```go
// https://github.com/golang/go/blob/a10e42f219abb9c5b/src/runtime/chan.go#L176
...
// https://github.com/golang/go/blob/a10e42f219abb9c5b/src/runtime/chan.go#L224
	if c.closed != 0 {
		unlock(&c.lock)
...
// https://github.com/golang/go/blob/a10e42f219abb9c5b/src/runtime/chan.go#L229
...
```go
// https://github.com/golang/go/blob/a10e42f219abb9c5b/src/runtime/chan.go#L318
...
// https://github.com/golang/go/blob/a10e42f219abb9c5b/src/runtime/chan.go#L236
...
// https://github.com/golang/go/blob/a10e42f219abb9c5b/src/runtime/chan.go#L257
...
https://github.com/golang/go/blob/a10e42f219abb9c5b/src/runtime/chan.go#L508

해당 함수를 따라가며 분석해보겠습니다. 먼저 nil 채널에 전달하면 에러 처리를 진행하고 고루틴의 작동을 중단합니다. 밑의 !block && empty(c)절은 당장은 무시합니다. 해당 코드는 나중에 확인해보겠습니다.

```go
// https://github.com/golang/go/blob/a10e42f219abb9c5b/src/runtime/chan.go#L524
...
닫힌 채널이면 qcount가 0일 때 수신자의 메모리를 초기화합니다. 채널이 열려 있는 경우에는 sendq에서 대기 중인 고루틴이 있는지 확인합니다.
채널에서 송신을 대기하는 고루틴이 차단된 경우입니다. 이 경우에 sendq라는 송신을 대기하는 고루틴 큐에서 대기 중인 고루틴을 가져옵니다. 이 부분이 나타내는 의미는 다음과 같습니다.
...
// https://github.com/golang/go/blob/a10e42f219abb9c5b/src/runtime/chan.go#L588
	if c.closed != 0 {
			if raceenabled {
...
```go
// https://github.com/golang/go/blob/a10e42f219abb9c5b/src/runtime/chan.go#L702
...
// https://github.com/golang/go/blob/a10e42f219abb9c5b/src/runtime/chan.go#L612
	if c.qcount > 0 {
...
// https://github.com/golang/go/blob/a10e42f219abb9c5b/src/runtime/chan.go#L636
