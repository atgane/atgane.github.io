---
title: "golang 비동기. synchronization primitive(1) sync.Map구현에서 적용해보는 sync.RWMutex"header:
...
golang 비동기 프로그래밍에서는 다양한 라이브러리가 사용됩니다. 이번에는 동기화를 맞추는데 가장 기본적으로 사용되는 sync.RWMutex를 이용하여 golang의 동시성에 대해 안전한 map인 generic map을 구현해보겠습니다. 해당 구조체에 sync.Map과 비슷한 동작을 구현해보겠습니다.
...
이번에는 sync.RWMutex입니다. RWMutex는 여러 개의 읽기 고루틴은 동시에 읽게 하고 쓰기에 대한 고루틴은 항상 단일로 동작하여 모든 읽기와 쓰기를 차단하도록 구현했습니다. sync.RWMutex는 크게 다음의 네 가지 메서드를 사용합니다.
...
아래 상태에서 Value의 타입이 비교 가능한 comparable이어야 사용할 수 있는 compare메서드를 제외한 나머지 메서드를 구현합니다.
...
동시성에 대하여 안전하게 구현하기 위해 golang 내부에서 sync.RWMutex를 활용할 때는 일반적으로 다음과 같이 처리합니다.
...
락을 이용하는 데 보다시피 큰 문제는 없습니다. 락을 획득하고 함수를 탈출할 시점에 defer를 이용하여 락을 해제하고 이후 로직을 처리하는 코드를 작성합니다.
...
이런 방식으로 generic map의 구현에서 RWMutex를 이용하여 단순한 방법으로 동시성에 안전한 map을 구현할 수 있습니다. 사실은 안전하지 않은 부분이 한 가지 있습니다. 어떤 문제인지 살펴봅시다. 

---

## 컬백에서 발생할 수 있는 데드락

데드락의 예시로 위에서 짠 코드로 어떤 시나리오를 가정해봅시다. 만약 웹 소켓을 이용한 채팅서버 코드에서 위에서 구현한 ds/Map을 이용하여 브로드캐스팅을 구현한 경우 데드락이 발생할 수 있습니다. 코드 예시를 들면, 다음과 같습니다. 코드 해설은 주석으로 간단하게 적어두었습니다.
...
이번에는 sync.Map에서도 비슷한 이슈가 발생하는 지 살펴보도록 합시다.
...
실행해보면 재밌는 결과를 확인할 수 있는데, 데드락이 발생하지 않고 동작함을 확인할 수 있습니다. 이는 어떻게 구현한 걸까요? 코드 내부를 탐사해봅시다.

---

## 약한 일관성과 sync.Map의 Range 내부

[sync.Map](http://sync.Map)내부에서는 그럼 어떻게 동작하기에 동시성 문제를 회피할 수 있었던 걸까요? 먼저 golang/go의 sync/map.go 내부에서 Map의 Range 코드 내부를 살펴봅시다. 
...
https://github.com/golang/go/blob/4e548f2c8e489a408033c8aab336077b16bc8cf7/src/sync/map.go#L120
...
https://github.com/golang/go/blob/4e548f2c8e489a408033c8aab336077b16bc8cf7/src/sync/map.go#L330
...
중요한 것은 sync.Map은 read와 dirty를 분리하고 read에서 읽기 전용으로 빠르게 읽고, 락이 걸려야 하는 작업은 dirty에 적절히 분산하여 해결하는 것입니다. 다시 [sync.Map](http://sync.Map)의 Range 연산을 살펴보면, 락을 획득하는 부분은 하나의 지점인 것을 알 수 있습니다. 바로 `read.amended` 가 참이 되어 `m.read.Store(&copyRead)` 로 read를 갱신하는 부분입니다. 그 뒤에 read.m을 순회하며 모든 k, v에 대하여 f를 실행하고 있는 것을 확인할 수 있습니다. 따라서 f에서 sync.Map을 변경하려고 하면, 즉 락을 획득하고 데이터에 create, update, delete와 관련된 연산을 진행된다면, 반영되지 않은 상태로 read가 복제된 시점의 스냅샷을 이용하여 동작한다는 것을 알 수 있습니다. 이렇게 약한 일관성을 가져 Range를 순회하는 read의 포인터에 대하여 해당 시점에 시점에 일어난 sync.Map에 대한 변화를 반영하지 않습니다. 그러나 다른 연산에 대해서는 강한 일관성을 보장합니다.
...
이렇게 컨테이너 형식의 자료구조에서 컬백함수로 여러 개의 대상을 실행시키는 코드가 존재하는 경우, **컬백함수 내부에서 락을 획득하여 실행하게 되면 컬백함수 내부에서 자료구조의 create/update/delete를 호출할 때 데드락이 발생할 수 있다는 것**을 알 게 되었습니다. 이런 경우, 회피하는 방법으로 **약한 일관성을 이용하여 컨테이너의 일부 또는 전체를 복사**하여 문제를 회피할 수 있었습니다. 실제로 golang의 sync.Map은 해당 방식으로 동작하고 있고, read와 dirty를 분리하여 읽기 성능을 가속화하고 있다는 점도 확인했습니다. 생각보다 golang의 내부는 분산 시스템의 구조가 보이는 것 같습니다.
