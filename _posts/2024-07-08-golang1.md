...
toc: true
toc_sticky: true
excerpt: "고루틴 스케줄러의 MPG 아키텍처(Machine, Process, Goroutine)와 동작 원리를 Go 내부 소스 코드를 분석하며 살펴봅니다."
author_profile: true
header:
...
![](/assets/images/posts/2024-07-08-1-1.png)

위의 그림은 [https://www.ardanlabs.com/blog/2018/08/scheduling-in-go-part2.html](https://www.ardanlabs.com/blog/2018/08/scheduling-in-go-part2.html) 에서 소개된 고루틴 스케줄러를 설명하는 그림중 일부분입니다. “go 100가지 실수 패턴과 솔루션”을 참고해보면 아주 간단하게 고루틴 스케줄러 동작을 다음과 같이 요약해 볼 수 있습니다.
...
![Untitled](/assets/images/posts/2024-07-08-1-2.png)

위의 그림은 [https://youtu.be/wQpC99Xu1U4?si=Hv1O5Hn7bWvSvxVa](https://youtu.be/wQpC99Xu1U4?si=Hv1O5Hn7bWvSvxVa) gophercon 2021의 GopherCon 2021: Madhav Jivrajani - Queues, Fairness, and The Go Scheduler에서 가져왔습니다. 고루틴이 golang 내부에서 어떻게 동작하는지 한눈에 보여주는 다이어그램이라고 생각합니다. 
...
![Untitled](/assets/images/posts/2024-07-08-1-3.png)
...
```mermaid
sequenceDiagram
    participant main as main p
    participant sub as sub p

    Note over main: main 함수에서 고루틴 생성
    main->>main: main 함수에서 고루틴 생성

    loop
		main->>main: runtime/newproc에서 로컬 큐 적재
    end

    main->>sub: runtime/wakep 호출

    sub->>sub: runtime/findrunnable 호출
    sub->>main: runtime/stealWork로 고루틴 훔치기
